...
ai <- readRDS(file.path(drv_root, "mediator_rxl_antiinflammatory_rx.rds"))
...
ad <- readRDS(file.path(drv_root, "mediator_rxl_antidepressant_rx.rds"))
...
opioids_ai <- 
    ungroup(opioids_ai) |> 
    mutate(mediator_opioid_antiinflamatory_copresc = 1) |> 
    select(-data)
...
opioids_ad <- 
    ungroup(opioids_ad) |> 
    mutate(mediator_opioid_antidepressant_copresc = 1) |> 
    select(-data)

## Benzo and gaba non-copresc

benzo <- readRDS(file.path(drv_root, "mediator_benzo_rx_bin.rds")) 
gaba <- readRDS(file.path(drv_root, "mediator_gabapentinoid_rx_bin.rds"))
mrelax <- readRDS(file.path(drv_root, "mediator_muscle_relaxant_rx_bin.rds"))

ai_mediator <- readRDS(file.path(drv_root, "mediator_antiinflammatory_rx_bin.rds"))
ad_mediator <- readRDS(file.path(drv_root, "mediator_antidepressant_rx_bin.rds"))

copresc <- readRDS(file.path(drv_root, "mediator_opioid_coprescriptions.rds")) |>
    left_join(benzo, by = c("BENE_ID" = "BENE_ID")) |>
    left_join(gaba, by = c("BENE_ID" = "BENE_ID")) |>
    left_join(mrelax, by = c("BENE_ID" = "BENE_ID")) |>
    left_join(ai_mediator, by = c("BENE_ID" = "BENE_ID")) |>
    left_join(ad_mediator, by = c("BENE_ID" = "BENE_ID")) |>
    left_join(opioids_ai, by = c("BENE_ID" = "BENE_ID")) |>
    left_join(opioids_ad, by = c("BENE_ID" = "BENE_ID")) 

nonoverlap <- copresc |>
    mutate(mediator_opioid_antiinflamatory_copresc = ifelse(is.na(mediator_opioid_antiinflamatory_copresc), 0, mediator_opioid_antiinflamatory_copresc),
           mediator_opioid_antidepressant_copresc = ifelse(is.na(mediator_opioid_antidepressant_copresc), 0, mediator_opioid_antidepressant_copresc)) |>
    mutate(mediator_nonopioid_nonoverlap_benzo = ifelse(mediator_opioid_benzo_copresc == 0 & mediator_benzo_rx == 1, 1, 0),
           mediator_nonopioid_nonoverlap_gaba = ifelse(mediator_opioid_gaba_copresc == 0 & mediator_gaba_rx == 1, 1, 0),
           mediator_nonopioid_nonoverlap_mrelax = ifelse(mediator_opioid_mrelax_copresc == 0 & mediator_nonopioid_muscle_relaxant_rx == 1, 1, 0),
           mediator_nonopioid_nonoverlap_antiinflamatory = ifelse(mediator_opioid_antiinflamatory_copresc == 0 & mediator_nonopioid_antiinflammatory_rx == 1, 1, 0),
           mediator_nonopioid_nonoverlap_antidepressant = ifelse(mediator_opioid_antidepressant_copresc == 0 & mediator_nonopioid_antidepressant_rx == 1, 1, 0)) |>
    select(BENE_ID, mediator_nonopioid_nonoverlap_benzo, mediator_nonopioid_nonoverlap_gaba, mediator_nonopioid_nonoverlap_mrelax,
           mediator_nonopioid_nonoverlap_antiinflamatory, mediator_nonopioid_nonoverlap_antidepressant)

saveRDS(nonoverlap, file.path(drv_root, "mediator_nonoverlap.rds"))
