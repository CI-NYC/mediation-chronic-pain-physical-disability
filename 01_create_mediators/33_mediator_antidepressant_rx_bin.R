# -------------------------------------
# Script: 33_mediator_antidepressant_rx_bin.R
...
ad_rxl <- 
    nop_rxl[, mediator_nonopioid_antidepressant_rx := fifelse(NDC %in% N06A$NDC, 1, 0)
    ][mediator_nonopioid_antidepressant_rx == 1, ]

ad_otl <- 
    nop_otl[, mediator_nonopioid_antidepressant_rx := fifelse(NDC %in% N06A$NDC, 1, 0)
    ][mediator_nonopioid_antidepressant_rx == 1, ]

# Save
saveRDS(ad_rxl, file.path(drv_root, "mediator_rxl_antidepressant_rx.rds"))
saveRDS(ad_otl, file.path(drv_root, "mediator_otl_antidepressant_rx.rds"))
...
ad[, mediator_nonopioid_antidepressant_rx := as.numeric(any(!is.na(NDC))), by = BENE_ID]
ad <- unique(ad[, .(BENE_ID, mediator_nonopioid_antidepressant_rx)])

saveRDS(ad, file.path(drv_root, "mediator_antidepressant_rx_bin.rds"))
