...
nop_pain <- merge(dts_cohorts, nop_pain, all.x = TRUE)

nop_pain[, mediator_nonopioid_pain_rx := as.numeric(any(!is.na(NDC))), by = BENE_ID]
nop_pain <- unique(nop_pain[, .(BENE_ID, mediator_nonopioid_pain_rx)])

saveRDS(nop_pain, file.path(drv_root, "mediator_nonopioid_pain_rx_bin.rds"))
