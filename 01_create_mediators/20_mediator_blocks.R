...
#   the analysis cohort had a claim for nerve or joint blocks
...
claims <- claims[, .(mediator_has_blocks = as.numeric(.N > 0)), by = "BENE_ID"]
...
claims[, mediator_has_blocks := fifelse(is.na(mediator_has_blocks), 0, mediator_has_blocks)]

saveRDS(claims, file.path(drv_root, "mediator_has_blocks.rds"))
