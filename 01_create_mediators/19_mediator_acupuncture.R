...
#   the analysis cohort had a claim for acupuncture
...
claims <- claims[, .(mediator_has_acupuncture = as.numeric(.N > 0)), by = "BENE_ID"]
...
claims[, mediator_has_acupuncture := fifelse(is.na(mediator_has_acupuncture), 0, mediator_has_acupuncture)]

saveRDS(claims, file.path(drv_root, "mediator_has_acupuncture.rds"))
