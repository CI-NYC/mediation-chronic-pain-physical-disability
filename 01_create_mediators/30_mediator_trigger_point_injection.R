...
#   the analysis cohort had a claim for trigger point injection
...
claims <- claims[, .(mediator_has_trigger_point_injection = as.numeric(.N > 0)), by = "BENE_ID"]
...
claims[, mediator_has_trigger_point_injection := fifelse(is.na(mediator_has_trigger_point_injection), 0, mediator_has_trigger_point_injection)]

saveRDS(claims, file.path(drv_root, "mediator_has_trigger_point_injection.rds"))
