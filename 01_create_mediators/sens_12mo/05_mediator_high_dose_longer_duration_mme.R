...
#               modified code originally written by Sarah Forrest and Nick
#               Williams in the 15_mediator_max_daily_dose_mme.R script
#        
...
all_opioids_clean_mediator_12mo_period <- all_opioids_clean_merged[
    all_opioids_clean_merged$rx_start_dt %within% interval(
        all_opioids_clean_merged$washout_12mos_end_dt, all_opioids_clean_merged$washout_12mos_end_dt + days(182)
    ), 
]
...
opioids <- all_opioids_clean_mediator_12mo_period[, list(data = list(data.table(.SD))), by = BENE_ID]
...
calculate_total_daily_dose <- function(data) {
    ...
    # Calculate the date limit based on washout_12mos_end_dt + 182 days
    ...
    ]#[, .(mediator_12mo_max_daily_dose_mme = max(total_mme_strength))]
}
...
dates <- dts_cohorts |>
    ...
    select(BENE_ID, washout_12mos_end_dt)
...
dates_complete <- map_dfr(split(dates, dates$BENE_ID),
                  function(x) {
                      tibble(BENE_ID = x$BENE_ID[1],
                             when = seq(min(x$washout_12mos_end_dt), min(x$washout_12mos_end_dt) + days(182), by = "day"))
                  })
...
consecutive_days <- function(data) {
    data |>
        ...
        distinct(BENE_ID) |>
        mutate(mediator_12mo_high_dose_longer_duration_mme = 1)
}
...
final[, mediator_12mo_high_dose_longer_duration_mme := fifelse(is.na(mediator_12mo_high_dose_longer_duration_mme), 0, mediator_12mo_high_dose_longer_duration_mme)]

# Save final dataset -----------------------------------------------------------

saveRDS(final, file.path(data_dir, "mediator_12mo_high_dose_longer_duration_mme.rds"))
