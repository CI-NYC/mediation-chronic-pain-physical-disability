...
#   the analysis cohort had a chiropractic claim
...
claims <- claims[, .(mediator_has_chiropractic = as.numeric(.N > 0)), by = "BENE_ID"]
...
claims[, mediator_has_chiropractic := fifelse(is.na(mediator_has_chiropractic), 0, mediator_has_chiropractic)]

saveRDS(claims, file.path(drv_root, "mediator_has_chiropractic.rds"))
