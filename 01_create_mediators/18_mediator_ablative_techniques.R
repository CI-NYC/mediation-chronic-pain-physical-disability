...
#   the analysis cohort had a claim for ablative techniques
...
claims <- claims[, .(mediator_has_ablative_techniques = as.numeric(.N > 0)), by = "BENE_ID"]
...
claims[, mediator_has_ablative_techniques := fifelse(is.na(mediator_has_ablative_techniques), 
                                          0, 
                                          mediator_has_ablative_techniques)]

saveRDS(claims, file.path(drv_root, "mediator_has_ablative_techniques.rds"))
arrange(claims, desc(mediator_has_ablative_techniques)
