...
#   the analysis cohort had a claim for epidural steroids
...
claims <- claims[, .(mediator_has_epidural_steroid = as.numeric(.N > 0)), by = "BENE_ID"]
...
claims[, mediator_has_epidural_steroid := fifelse(is.na(mediator_has_epidural_steroid), 0, mediator_has_epidural_steroid)]

saveRDS(claims, file.path(drv_root, "mediator_has_epidural_steroid.rds"))
