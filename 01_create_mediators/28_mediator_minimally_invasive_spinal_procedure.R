...
#   the analysis cohort had a claim for minimally invasive spinal procedures
...
codes <- read_yaml("0x_create_mediators/mediator_codes.yml")
...
claims <- claims[, .(mediator_has_minimally_invasive_spinal_procedure = as.numeric(.N > 0)), by = "BENE_ID"]
...
claims[, mediator_has_minimally_invasive_spinal_procedure := fifelse(is.na(mediator_has_minimally_invasive_spinal_procedure), 0, mediator_has_minimally_invasive_spinal_procedure)]

saveRDS(claims, file.path(drv_root, "mediator_has_minimally_invasive_spinal_procedure.rds"))
