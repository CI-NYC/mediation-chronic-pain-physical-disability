...
disability <- readRDS(paste0(drv_root, "/subset_disability_only_ref_df.rds")) |>
    mutate(anxdep_post_exposure_cal = 
               ifelse(anxiety_post_exposure_cal == 1 | depression_post_exposure_cal == 1 |
                          bipolar_post_exposure_cal == 1 |
                          mediator_has_counseling == 1, 
                      1, 0))

disability_subset <- disability |>
    filter(is.na(exposure_disability_only_subset) == FALSE)

disability_depanx <- disability |> # filtering to those with dep/anx/bipolar
    filter(depression_washout_cal == 1 |
               anxiety_washout_cal == 1 |
               bipolar_washout_cal == 1)

disability_only_no_depanx_subset <- disability |> # filtering to those without dep/anx/bipolar
    filter(depression_washout_cal == 0 &
               anxiety_washout_cal == 0 &
               bipolar_washout_cal == 0) |>
    filter(is.na(exposure_disability_only_subset) == FALSE)

disability_only_no_depanx <- disability |> # filtering to those without dep/anx/bipolar
    filter(depression_washout_cal == 0 &
               anxiety_washout_cal == 0 &
               bipolar_washout_cal == 0)
...
# Has at least one of anxiety, depression, or bipolar
W <- c(
    "dem_age",
    # "dem_sex",
    "dem_sex_m",
...
mediator_included <- "all_mediators_depanx"
...
M <- c("mediator_max_daily_dose_mme", #MME
       ...
       "mediator_nonopioid_nonoverlap_mrelax", #nonopioid
       "mediator_nonopioid_nonoverlap_gaba", #nonopioid
       "mediator_nonopioid_nonoverlap_benzo", #nonopioid
       "mediator_nonopioid_nonoverlap_antidepressant", #nonopioid
       "mediator_nonopioid_nonoverlap_antiinflamatory", #nonopioid
...
for (exposure in A) {
    ...
    saveRDS(result, paste0("/mnt/general-data/disability/mediation_unsafe_pain_mgmt/mediation_results_final_r1/all_mediators/expansive/", mediator_included, "_results_", exposure, ".rds"))
}

# # Has none of depression, anxiety, bipolar --------------------------------------------------
...
mediator_included <- "all_mediators_nodepanx"
...
mediator_included <- "all_mediators_depanx_icd"
...
for (exposure in A) {
    ...
    saveRDS(result, paste0("/mnt/general-data/disability/mediation_unsafe_pain_mgmt/mediation_results_final_r1/all_mediators/icd/", mediator_included, "_results_", exposure, ".rds"))
}
...
mediator_included <- "all_mediators_nodepanx_icd"
