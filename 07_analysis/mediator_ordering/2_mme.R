...
dts_cohorts <- readRDS(file.path(data_dir, "mediation_analysis_df_subsetted.rds"))
setDT(dts_cohorts)
...
cohort_longer_duration_COMPLETE2 <- cohort_longer_duration_COMPLETE |>
    ungroup() |>
    as.data.table()
# Save final dataset -----------------------------------------------------------
claims2 <- cohort_longer_duration_COMPLETE2[, .(high_dose_dt = min(date)), by = BENE_ID] 


saveRDS(claims2, "/mnt/general-data/disability/mediation_unsafe_pain_mgmt/mediation_first_dates/high_dose_first_dates.rds")
