...
## -----------------------------------------------------------------------------------------------------------
directory <- "/mnt/general-data/disability/mediation_unsafe_pain_mgmt/mediation_results_final_r1/depanx_icd"
...
directory <- "/mnt/general-data/disability/mediation_unsafe_pain_mgmt/mediation_results_final_r1/nodepanx_icd"
...
results <- rbind(results_depanx, results_nodepanx) |>
    mutate(
        ...
        mediator_included = case_when(
            mediator_included == "high_dose_longer_duration_mme" ~ "High Dose, Long Duration MME",
            ...
            mediator_included == "nonopioid_nonoverlap_mrelax_gaba_benzo" ~ 
                "Benzo/Gaba/Muscle Relaxant Presc w/o Opioid",
            mediator_included == "nonopioid_nonoverlap_antidepressant_antiinflamatory" ~
...
saveRDS(results, "~/mediation-chronic-pain-physical-disability/08_results/group_mediators_icd.rds")
...
suppressWarnings(group_plot)
ggsave("~/mediation-chronic-pain-physical-disability/10_figures/final_figs/group_mediators_icd.pdf", group_plot, dpi = 300, height = 8, width = 9)


## -----------------------------------------------------------------------------------------------------------
directory <- "/mnt/general-data/disability/mediation_unsafe_pain_mgmt/mediation_results_final_r1/depanx"
...
results_depanx$mediator_included <- str_extract(res, "(?<=mediator_).*?(?=_results)")
results_depanx$exposure <- str_extract(res, "(?<=results_exposure_).*?(?=_depanx)")
results_depanx$type <- "depanx"

directory <- "/mnt/general-data/disability/mediation_unsafe_pain_mgmt/mediation_results_final_r1/nodepanx"
...
results_nodepanx$mediator_included <- str_extract(res, "(?<=mediator_).*?(?=_results)")
results_nodepanx$exposure <- str_extract(res, "(?<=results_exposure_).*?(?=_nodepanx)")
results_nodepanx$type <- "nodepanx"
...
saveRDS(results, "~/mediation-chronic-pain-physical-disability/08_results/group_mediators.rds")
...
suppressWarnings(group_plot)
ggsave("~/mediation-chronic-pain-physical-disability/10_figures/final_figs/group_mediators.pdf", group_plot, dpi = 300, height = 8, width = 9)
